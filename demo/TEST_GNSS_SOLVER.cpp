...
NavTimeUTC parse_nav_timeutc(const uint8_t* payload, size_t length) {
    if (length != 20) {
...
PephBDS parse_beidou_ephemeris(const uint8_t* payload, size_t length) {
    if (length != 92) {
...
PalmBDS parse_beidou_almanac(const uint8_t* payload, size_t length) {
    if (length != 35) {
